%% ============================================================
%% Breaking the Bubble? — REVISED DRAFT (July 2026)
%% Results restructured around three claims; Methods replaced with
%% the validated post-fix description; model overview added before
%% Results for readability (Nature-style methods-last format).
%%
%% NOTES FOR MERGING INTO THE OVERLEAF PROJECT:
%% 1. Citation keys follow author+year+keyword. Keys marked
%%    %% [EXISTING] should already be in your .bib — align key names.
%%    Keys marked %% [NEW] are provided in new_references.bib.
%% 2. Two items are flagged [TO RE-RUN] in the text (cognitive-gap
%%    sweep, factor sweeps): cached results predate the C8/AECI-Var
%%    fixes and must be regenerated (and a decision taken on whether
%%    they run under the main configuration) before submission.
%% 3. Figure files referenced: comparison_configs.png,
%%    goldilocks_alignment_sweep.png, alpha_star_sensitivity.png,
%%    echo_chamber_lifecycle.png; supplementary: bubble_timeseries.png,
%%    transition_timing.png, periphery_gap.png, periphery_gap_evolution.png.
%% ============================================================

\documentclass[sn-apacite]{sn-jnl}
...
\abstract{Crises change human information collection, sharing, and decision-making. The combination of urgency, uncertainty, and high stakes is known to induce cognitive biases, informational bubbles and echo chambers. Trustworthy Artificial Intelligence (AI) promises to optimise decisions and reduce informational bottlenecks. Yet introducing AI as a `trusted' actor in crises fundamentally changes human interaction, information processing and decision-making. While the literature on human-centred AI focuses on the impact of AI on individual biases or decisions, what is missing is an understanding at scale of how AI shapes information processing, opinions and decisions under the volatile and uncertain conditions typical of disasters.
This paper uses an agent-based model to examine how an AI whose reports can be tuned towards human beliefs reshapes collective information dynamics in disaster response. Human agents with exploratory or exploitative information strategies select sources through Q-learning and revise beliefs through Bayesian updating, while an alignment parameter interpolates AI reports between ground truth and confirmation of the querying community's beliefs. We contrast a structurally realistic configuration, in which information access is bounded by a spatially embedded social network and agent mobility, with a control in which any agent can query any other.
Three findings emerge. First, network-bounded access is a structural precondition for AI-amplified echo chambers: confirming AI deepens community-level belief convergence only when queries follow network ties. Second, an interior optimum of AI alignment ($\alpha^{*} \approx 0.4$--$0.6$) that jointly minimises social and AI-induced echo chambers exists only under the realistic configuration; in the control, echo-chamber indices are minimised by full confirmation. Third, the realistic configuration is operationally more robust: belief accuracy and relief targeting degrade more slowly with alignment, and the operational collapse under near-full confirmation is buffered by roughly a factor of three. Structural realism thus shifts the harm of sycophantic AI from operational failure towards epistemic insularity, with direct implications for the design and monitoring of AI in crisis response.}
...
Increasingly, we humans rely on artificial intelligence (AI) to collect and analyse information and make decisions \citep{klingbeil2024trust}. %% [EXISTING]
This trend is particularly pronounced during crises, where decision-makers are confronted with highly complex decisions that need to be addressed urgently, leaving limited time for reflection and deliberation \citep{mendonca2001decision}. %% [EXISTING]
This combination is known to deeply impact human information processing, sharing and decision-making behaviour \citep{comes2020coordination}. %% [EXISTING]
To help humans manage this complexity, AI is promoted as a tool to optimise decisions and information flows \citep{jeon2024hearhere}. %% [EXISTING]
...
A prerequisite to the use of technology in crises is trust \citep{steinbrink2024transparency}. %% [EXISTING]
In the field of AI, there is a plethora of guidelines and approaches for designing AI that is trustworthy, especially in the context of crises and disasters; for a recent review, see \citet{albahri2024systematic}. %% [EXISTING]
Often, the trust relationship is conceptualised as one individual trusting one AI, whereby it is claimed that trust is promoted by more transparency \citep{visave2025transparency}. %% [EXISTING]
Others focus on public trust as the result of a design and decision-making process \citep{tzachor2020artificial}. %% [EXISTING]
Humans -- especially in crises -- have a tendency towards selective exposure \citep{barbera2015tweeting} and confirmation bias \citep{paulus2024interplay}, whereby they reject conflicting information. %% [EXISTING]

Increasingly, generative AI has started to mimic human behaviour and tune to the opinions of its users \citep{sharma2024generative} as a way to gain acceptance and `trust'. %% [EXISTING]
This introduces a fundamental dilemma: should AI provide accurate information -- even if it contradicts beliefs and is potentially rejected -- or should it align with human beliefs to foster immediate acceptance and trust? If AI overly tunes into existing beliefs to gain rapid trust, it risks amplifying cognitive biases and echo chambers \citep{ekstrom2022self}. %% [EXISTING]
...
While studies on the implications of black-box generative models have started to emerge \citep{sharma2024generative,zhang2024see}, %% [EXISTING]
a gap in current research is understanding how these interactions of individuals with AI scale up to broader social network dynamics during crises, potentially creating or exacerbating polarisation and filter bubbles. Recent experimental work shows that dyadic human--AI feedback loops amplify individual biases more strongly than human--human interaction \citep{glickman2024human}, %% [NEW]
yet collective phenomena such as echo chambers are properties of networks rather than of dyads: social influence in networks is known to reshape collective accuracy in ways that individual-level analysis cannot reveal \citep{lorenz2011social,becker2017network}. %% [NEW]
...
Fuelled by climate change, increasing inequalities, conflict and war, we are confronted with global poly-crises that erode progress towards sustainability \citep{sogaard2024evolution}. %% [EXISTING]
As crises require rapid interventions that overwhelm human decision-making capacity, digital tools have been portrayed as a potential avenue to automatically collect and analyse data to support or make decisions \citep{qadir2016crisis}. %% [EXISTING]
Examples range from community mapping \citep{champlin2023measuring,soden2014crowdsourced}, the use of aerial images, remote sensing, UAVs and digital platforms for better crisis management \citep{voigt2007satellite} to increasing automation of the delivery of aid, facilitated by mobile phone and blockchain technology \citep{baharmand2019leveraging,coughlan2015forecast}. %% [EXISTING]
Despite these technological advances, the emergent nature of crises poses challenges for information collection, sharing, and decision-making \citep{nespeca2020towards,turoff2004design}. %% [EXISTING]
...
AI in combination with data collection via remote sensing or mobile technologies promises a broader perspective, addressing both the network density and the information intensity that often lack in human social networks \citep{pan2012crisis}. %% [EXISTING]
The promise of real-time data collection, especially in highly dynamic situations, is said to allow decision-makers to identify critical tipping points rapidly, before crises spiral out of control. For almost two decades, the focus in this arena has been on social media and real-time surveillance via mobile phones to rapidly understand sentiment, perception, or human movement patterns \citep{mai2025evaluating,middleton2013real,qadir2016crisis}. Following the rapid progress in remote sensing, there are now also increasing applications in natural hazard monitoring, including floods \citep{zhou2024threshold} and wildfires \citep{roy2024multi}. %% [EXISTING]
...
Periphery analyses -- decomposing outcomes by distance to the epicentre and by network centrality -- are reported in Supplementary Figures~S3 and~S4; the structural scope for periphery gaps differs by construction between the two configurations (Methods), and we do not build claims on them here.
...
Alternative formulations based on opinion fragmentation \citep{bail2018exposure} or information entropy \citep{sasahara2021social} %% [NEW]
...
Supplementary Materials S1--S9 contain the full parameter tables, metric definitions, experiment designs and supplementary figures referenced in the text.
...
\bibliography{new_references}%% match to the project's .bib file name
